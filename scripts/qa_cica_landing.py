"""Read-only browser checks for the local CICA marketing page."""
...
with sync_playwright() as p:
    ...
    assert page.locator("body").get_attribute("data-cica-build") == "spatial-v6"
    ...
    for width, height, label in [
        ...
        if not page.evaluate("document.documentElement.scrollWidth <= innerWidth"):
            print(
                page.evaluate(
                    "Array.from(document.querySelectorAll('*')).filter(e=>e.getBoundingClientRect().right>innerWidth+1).map(e=>[e.tagName,e.className,e.getBoundingClientRect().right])"
                )
            )
        assert page.evaluate("document.documentElement.scrollWidth <= innerWidth"), label
        page.screenshot(path=str(Path(gettempdir()) / f"cica-final-{label}.png"))
        page.locator("#demonstracao").scroll_into_view_if_needed()
        page.screenshot(path=str(Path(gettempdir()) / f"cica-final-{label}-demo.png"))
        page.locator("#modulos").scroll_into_view_if_needed()
        page.screenshot(path=str(Path(gettempdir()) / f"cica-final-{label}-modules.png"))
    ...
    browser.close()
print(
    "PASS: uncluttered hero, scenarios, evidence, focus return, 4 viewports, "
    "reduced motion, FAQ, lead navigation."
)
