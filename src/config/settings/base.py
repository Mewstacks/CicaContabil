...
CICA_CLAUDE_API_KEY = env_str("CICA_CLAUDE_API_KEY")

# The product owns provider apps; each office only consents to its own mailbox.
# Keep both providers off until Mewstack has registered and homologated the apps.
TRIAGE_OAUTH_BASE_URL = env_str("TRIAGE_OAUTH_BASE_URL")
TRIAGE_MS_OAUTH_ENABLED = env_bool("TRIAGE_MS_OAUTH_ENABLED", False)
TRIAGE_MS_CLIENT_ID = env_str("TRIAGE_MS_CLIENT_ID")
TRIAGE_MS_CLIENT_SECRET = env_str("TRIAGE_MS_CLIENT_SECRET")
