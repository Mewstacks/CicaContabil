...
O estado vigente está no plano mestre: as etapas 00–03 foram concluídas no nível local e a etapa 04 (Siescon) está em andamento, aguardando o contrato técnico Q-33. A limitação original à etapa 00 permanece apenas no histórico. Consulte os registros canônicos antes de perguntar regras já decididas ou iniciar implementação.
...
| Conciliação OFX × Domínio | Entrada local limitada de OFX/CSV/XLSX/PDF, mapeamento revisável, partidas equilibradas, evidência de conciliação, reexportação auditável e recuperação de execução | Aprovar layouts, habilitar OCR português, medir corpus/volume e conferir a importação em Domínio/Siescon por escritório/empresa |
| Radar da Reforma | Coleta local idempotente de fontes oficiais fixas, alertas com links e estado seguro de falha por fonte | Provar rotina, frescor, falha e recuperação nas fontes reais em ambiente operacional |
